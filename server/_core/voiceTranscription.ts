...
export type TranscribeFromBufferOptions = {
...
/**
 * Transcribe audio directly from Buffer (no URL download needed)
 * This is more reliable for mobile as it avoids S3 URL fetch issues
 */
export async function transcribeFromBuffer(
  ...
  try {
    ...
    console.log(`[transcribeFromBuffer] Processing audio: ${audioBuffer.length} bytes, type: ${mimeType}`);
    ...
    console.error(`[transcribeFromBuffer] Error:`, error);
...
export async function transcribeAudio(
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`[transcribeAudio] Download failed: ${response.status} ${response.statusText}`);
        return {
      ...
      console.log(`[transcribeAudio] Downloaded: ${audioBuffer.length} bytes, type: ${mimeType}`);
      ...
      console.error(`[transcribeAudio] Fetch error:`, error);
      return {
    ...
    // Use the buffer-based transcription
    return transcribeFromBuffer({
      audioBuffer,
      mimeType,
